  ...
  return (
    ...
    <div className='bg-white shadow-lg rounded-lg p-0 lg:p-8 pb-12 mb-8'>
      ...
      <div className='block lg:flex text-center items-center justify-center mb-8 w-full'>
        ...
        <div className='font-medium text-gray-700'>
          ...
          <span>{moment(post.createdDate).format('MMM DD, YYYY')}</span>
        </div>
      </div>
      <p className='text-center text-lg text-gray-700 font-normal px-4 lg:p20 mb-8'>{post.excerpt}</p>
    ...
    {/* <div className='bg-white shadow-lg rounded-lg p-0 lg:p-8 pb-12 mb-8'>
      <div className='flex flex-row'>
        <div className='relative overflow-hidden shadow-md pb-80 w-80 flex-none'>
          <img src={post.featuredImage.url} alt={post.title} className="object-top absolute h-full w-80 object-cover shadow-lg rounded-t-lg lg:rounded-lg" />
        </div>
        <div className='ml-4 relative w-full'>
          <div className='mb-8'>
            {post.categories.map((category, index) => (
              <span className={`bg-pink-600 text-sm text-white p-1 ${index>0 ? 'ml-2': ''} rounded-md`}>{category.name}</span>
            ))}
          </div>
          ...
          <p className=' text-lg text-gray-700 font-normal lg:p20 mb-8'>{post.excerpt}</p>
          <div className='block lg:flex text-center items-center w-full absolute inset-x-0 bottom-0'>
            <div className='flex items-center justify-center mb-4 lg:mb-0 w-full lg:w-auto mr-8'>
              <img src={post.author.photo.url} alt={post.author.name} height="30px" width="30px" className='align-middle rounded-full' />
              <p className='inline align-middle text-gray-700 ml-2 text-lg'>{post.author.name}</p>
            </div>
